...
RSpec.describe Article, type: :model do
  describe '#initialization' do
    ...
    it 'should create the article' do
      article = Article.new(valid_rss_article, feed)
      expect(article.title).to eq('title')
      expect(article.link).to eq('link')
      expect(article.date).to eq('11-11-2011')
      expect(article.channel).to eq(feed.title)
    ...
    it 'should raise an error when rss article is invalid' do
      expect { Article.new(invalid_rss_article, feed) }.to raise_error(ArgumentError)
    end
  end
end
